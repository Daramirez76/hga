...
const viewUserProfile = document.getElementById('viewUserProfile');
...
function setupProfileMenu() {
  ...
  document.addEventListener('click', (event) => {
    if (!profileDropdown.contains(event.target) && !profileMenuButton.contains(event.target)) closeProfileDropdown();
  });
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') closeProfileDropdown();
  });
  if (viewUserProfile) viewUserProfile.addEventListener('click', () => { window.location.href = 'user.html'; });
  if (logoutUser) logoutUser.addEventListener('click', () => { logoutSession(); });
}
