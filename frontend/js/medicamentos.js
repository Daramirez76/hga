...
function abrirModalNuevoMedicamento() {
  window.HgaAlerts.info('Se implementara un modal completo para agregar medicamentos\n\nCampos requeridos:\n- Nombre\n- Fecha entrada (YYYY-MM-DD)\n- Fecha vencimiento (YYYY-MM-DD)\n- Stock\n- Descripcion de novedad (opcional)', 'Proximamente');
}
...
async function registrarEntrega(id) {
  const respuesta = await window.HgaAlerts.prompt('¿Cuantas unidades se entregan?', {
  ...
  const cantidad = respuesta.value;
  ...
  try {
    ...
    await window.HgaAlerts.success('Entrega registrada correctamente');
    cargarMedicamentos();
...
async function eliminarMedicamento(id) {
  if (!(await window.HgaAlerts.confirm('¿Estas seguro de que deseas eliminar este medicamento?', 'Eliminar medicamento'))) {
    return;
  }
