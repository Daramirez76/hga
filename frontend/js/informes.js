...
async function guardarInforme() {
  if (!inputCodigo.value || !inputDoc.value || !inputResidente.value || !inputTitulo.value || !inputRol.value) {
    await window.HgaAlerts.warning('Completa todos los campos', 'Formulario incompleto');
    return;
  }
  ...
  try {
    ...
    await window.HgaAlerts.success('Informe creado');
    ...
    await window.HgaAlerts.error('Error al crear informe: ' + error.message);
...
async function eliminarInforme(id) {
  if (!(await window.HgaAlerts.confirm('¿Seguro deseas eliminar el informe?', 'Eliminar informe'))) return;
...
function editarInforme(informe) {
  ...
  botonGuardar.onclick = async function() {
    try {
      ...
      const datos = await res.json();
      if (!res.ok) throw new Error(datos.message || `Error HTTP ${res.status}`);
      await window.HgaAlerts.success('Informe actualizado');
      modal.hide();
      cargarInformes();
    } catch (error) {
      console.error('Error actualizar informe:', error);
      await window.HgaAlerts.error('No se pudo actualizar: ' + error.message);
    } finally {
      botonGuardar.textContent = 'Guardar';
