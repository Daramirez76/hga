  ...
  if (form) {
    ...
    if (btnGestionar) {
      btnGestionar.addEventListener('click', async function(e) {
        ...
        if (!todosValidos) {
          await window.HgaAlerts.warning('Por favor completa todos los campos de los residentes', 'Formulario incompleto');
          return false;
        }
        ...
        try {
          ...
          await window.HgaAlerts.error(`Error al guardar: ${error.message}`);
        } finally {
