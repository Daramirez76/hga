...
async function fetchJson(path, method = 'GET', allow404 = false) {
...
function countUniqueActiveUsers(resources, currentUser) {
  ...
  if (currentUser && currentUser.id) {
    userIds.add(String(currentUser.id));
  }
...
function updateMetricCard(metricKey, percent, count, detailText) {
  ...
  if (countNode) {
    countNode.textContent = formatCount(count, label);
  }
  if (detailNode) {
...
function buildFallbackSnapshot(monthValue) {
...
function getApiMetric(metricKey, monthValue) {
  const metrics = dashboardState.dashboardData?.metrics || {};

  if (monthValue === 'all') {
    return Number((metrics.all && metrics.all[metricKey]) ?? 0);
  }

  const byMonth = metrics.by_month || {};
  const monthKey = String(monthValue).padStart(2, '0');
  const monthMetrics = byMonth[monthKey] || byMonth[String(Number(monthValue))] || {};
  return Number(monthMetrics[metricKey] ?? metrics.all?.[metricKey] ?? 0);
}

function buildApiSnapshot(monthValue) {
  return {
    filteredResources: dashboardState.resources,
    metrics: {
      residentes: getApiMetric('residentes', monthValue),
      medicamentos: getApiMetric('medicamentos', monthValue),
      actividades: getApiMetric('actividades', monthValue),
      citas: getApiMetric('citas', monthValue),
      visitas: getApiMetric('visitas', monthValue),
      usuarios: getApiMetric('usuarios', monthValue),
      informes: getApiMetric('informes', monthValue),
    },
  };
}
...
function renderLineChartFallback(moduleKey, monthValue) {
  const months = [];
...
function renderLineChartApi(moduleKey, monthValue) {
  const chartSeries = dashboardState.dashboardData?.charts?.last_six_months || [];
  const hasSpecificModule = moduleKey !== 'all';
  const chartTitle = hasSpecificModule ? (RESOURCE_CONFIG[moduleKey]?.label || 'Modulo') : 'Actividad mensual';
  trendTitle.textContent = hasSpecificModule ? `Actividad mensual - ${chartTitle}` : 'Actividad mensual';
  trendCaption.textContent = monthValue === 'all' ? 'Ultimos 6 meses' : `Contexto del filtro: ${MONTH_NAMES[Number(monthValue) - 1]}`;
  ...
  const series = chartSeries.map((entry) => {
    const totals = entry && typeof entry.totals === 'object' ? entry.totals : {};
    if (hasSpecificModule) {
      return Number(totals[moduleKey] ?? 0);
    }
    return Number(entry.activity_total ?? Object.values(totals).reduce((sum, value) => sum + Number(value || 0), 0));
  });

  if (!series.some((value) => value > 0)) {
    lineChartStage.innerHTML = '<div class="chart-empty">No hay datos suficientes para graficar este modulo.</div>';
    return;
  }
  ...
  const points = series.map((value, index) => {
    const x = padding.left + (usableWidth / Math.max(series.length - 1, 1)) * index;
    const y = padding.top + usableHeight - (value / maxValue) * usableHeight;
    return { x, y, value, label: chartSeries[index]?.label || '' };
  });
  const pointList = points.map((point) => `${point.x},${point.y}`).join(' ');
  const areaList = `${padding.left},${height - padding.bottom} ${pointList} ${width - padding.right},${height - padding.bottom}`;
  const axisLabels = points.map((point, index) => `<text x="${point.x}" y="${height - 12}" text-anchor="middle" font-size="10" fill="#5f7074">${escapeHtml(String(chartSeries[index]?.label || ''))}</text>`).join('');
  const pointMarkers = points.map((point) => `<circle cx="${point.x}" cy="${point.y}" r="3.5" fill="#ffffff" stroke="#2a4f56" stroke-width="2"></circle><text x="${point.x}" y="${point.y - 10}" text-anchor="middle" font-size="10" fill="#2a4f56">${point.value}</text>`).join('');

  lineChartStage.innerHTML = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Grafico de actividad mensual"><line x1="${padding.left}" y1="${height - padding.bottom}" x2="${width - padding.right}" y2="${height - padding.bottom}" stroke="#c1d8d6" stroke-width="1.5"></line><line x1="${padding.left}" y1="${padding.top}" x2="${padding.left}" y2="${height - padding.bottom}" stroke="#c1d8d6" stroke-width="1.5"></line><polygon points="${areaList}" fill="rgba(53, 191, 210, 0.16)"></polygon><polyline points="${pointList}" fill="none" stroke="#2a4f56" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"></polyline>${axisLabels}${pointMarkers}</svg>`;
}

function renderPieChartFallback(snapshot, monthValue) {
...
function renderPieChartApi(monthValue) {
  const metrics = buildApiSnapshot(monthValue).metrics;
...
function renderDashboard() {
  const monthValue = monthFilter ? monthFilter.value || 'all' : 'all';
  const moduleValue = moduleFilter ? moduleFilter.value || 'all' : 'all';
  const snapshot = dashboardState.source === 'api' ? buildApiSnapshot(monthValue) : buildFallbackSnapshot(monthValue);

  renderMetrics(snapshot, monthValue);

  if (dashboardState.source === 'api') {
    renderLineChartApi(moduleValue, monthValue);
    renderPieChartApi(monthValue);
  } else {
    renderLineChartFallback(moduleValue, monthValue);
    renderPieChartFallback(snapshot, monthValue);
  }
...
async function loadDashboardEndpoint() {
  const payload = await fetchJson(DASHBOARD_ENDPOINT, 'GET', true);
  ...
  dashboardState.dashboardData = normalized;
  dashboardState.source = 'api';

  if (normalized.current_user) {
    dashboardState.currentUser = normalized.current_user;
    localStorage.setItem('usuario', JSON.stringify(normalized.current_user));
  }
...
async function initializeDashboard() {
  ...
  try {
    const dashboardPayload = await loadDashboardEndpoint();

    if (!dashboardPayload) {
      dashboardState.source = 'fallback';
